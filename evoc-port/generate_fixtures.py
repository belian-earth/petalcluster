...
def arr(x):
    ...
    Non-finite values become string sentinels: bare Infinity/NaN are not valid
    JSON and strict parsers (serde_json) reject them.
    """
    a = np.asarray(x)
    if a.dtype.kind != "f":
        return a.tolist()
    ...
    return enc(a.astype(np.float64).tolist())
...
def fixture_tree_seams(name, embedding):
    ...
    # S3: the whole deterministic clustering half, generated by replicating
    # build_cluster_layers step for step FROM THE CANONICAL MST. Running
    # build_cluster_layers directly would chain from the reference's internal
    # Boruvka emission order, which is not stable and (under mutual-reachability
    # weight degeneracy) not even a unique tree -- the fixture must be a pure
    # function of what it stores.
    n_threads_unused = n_threads  # kept for clarity; the chain below is serial
    linkage_c = mst_to_linkage_tree(sorted_mst)
    base_mcs = 10
    condensed_c = condense_tree(linkage_c, base_mcs)
    leaves_c = extract_leaves(condensed_c)
    base_labels = get_cluster_label_vector(condensed_c, leaves_c, 0.0, n)
    base_strengths = get_point_membership_strength_vector(condensed_c, leaves_c, base_labels)

    layers = [base_labels]
    strengths_layers = [base_strengths]
    scores = [0.0]

    mask_c = condensed_c.child >= n
    cluster_tree_c = mask_condensed_tree(condensed_c, mask_c)
    if len(cluster_tree_c.child) > 0 and cluster_tree_c.child[-1] >= n:
        births, deaths, parents_c, lambda_deaths = min_cluster_size_barcode(
            cluster_tree_c, n, base_mcs
        )
        sizes_c, total_persistence = compute_total_persistence(births, deaths, lambda_deaths)
        peaks_c = find_peaks(total_persistence)
        selected = select_diverse_peaks(
            peaks_c, total_persistence, sizes_c, births, deaths,
            min_similarity_threshold=0.2, max_layers=9,
        )
        for peak in selected:
            best_birth = sizes_c[peak]
            sel = np.where((births <= best_birth) & (deaths > best_birth))[0] + n
            labels_p = get_cluster_label_vector(condensed_c, sel, 0.0, n)
            strengths_p = get_point_membership_strength_vector(condensed_c, sel, labels_p)
            layers.append(labels_p)
            strengths_layers.append(strengths_p)
            scores.append(float(total_persistence[peak]))

    order = np.argsort([l.max() + 1 for l in layers], kind="stable")[::-1]
    layers = [layers[i] for i in order]
    strengths_layers = [strengths_layers[i] for i in order]
    scores = [scores[i] for i in order]

    write(f"{name}__cluster_layers", {
        "input": {
            "sorted_mst": arr(sorted_mst.astype(np.float64)),
            "n_samples": n,
            "min_samples": 5, "base_min_cluster_size": base_mcs,
            "min_similarity_threshold": 0.2, "max_layers": 10,
        },
        "output": {
            "layers": [arr(l) for l in layers],
            "strengths": [arr(sv) for sv in strengths_layers],
