...
- **Ward**: 2.5 to 4.5 times faster than base R and about 2 times faster
  ...
  raw data, `shoal_hclust()` writes the distances straight into the
  buffer the clustering consumes, so at 50,000 points it holds one 10 GB
  vector where `stats::dist()` plus `stats::hclust()` holds two, and
  that saved copy is most of the gap.
