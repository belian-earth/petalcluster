...
cap and linkage. Timings are medians of three runs on 20 cores and 62 GB,
from 500 to 50,000 points, on Gaussian blobs in 2 and 10 dimensions and, for
EVoC, on embedding-like data in 48. Scripts and data generation are in
`bench/`; the 50,000-point Ward rows need about 30 GB of RAM.
...
- **Ward**: 1.8 to 2 times faster than base R and 1.3 to 1.5 times faster
  than SciPy. All three run the same nearest-neighbour-chain algorithm, so the
  differences are in computing the distances that feed it. At 50,000 points
  the distance vector is 10 GB, and Ward holds two of them.
