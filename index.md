...
[`as.dendrogram()`](https://rdrr.io/r/stats/dendrogram.html) and
[`cmdscale()`](https://rdrr.io/r/stats/cmdscale.html) work without glue.
...
The density-based algorithms have no
[`predict()`](https://rdrr.io/r/stats/predict.html) method, because the
...
Both plots come from the same
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) method: the
shared class is what makes the comparison a one-liner. Noise points are
drawn as grey crosses.
...
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) draws a scatter
plot matrix. Pass `xcol` and `ycol` to pick a single pair instead. Here
R’s `quakes` data, 1,000 seismic events near Fiji, clustered on
location, depth and magnitude, is plotted by longitude and latitude.
...
![](reference/figures/README-quakes-1.png)
...
builds a nearest-neighbour graph, learns a compact node embedding from
it and clusters that at several granularities at once. Every layer is
returned, with a persistence score, and `layer` chooses which one
populates `cluster`.
...
- **Distances**:
  ...
  [`stats::dist()`](https://rdrr.io/r/stats/dist.html) in 2 dimensions
  and 4.5 times in 10, and level with SciPy’s `pdist()`, which edges it
  in 2 dimensions where the work is almost all memory traffic. The
