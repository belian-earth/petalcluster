# Benchmark: shoal vs dbscan R package
...
bench_r <- function(d) {
  ...
  hdbscan_rows <- lapply(seq_along(sizes), function(i) {
    ...
    t_petal <- time_it(shoal_hdbscan(x, min_samples = 5L, min_cluster_size = 15L))
    if (n > 20000L) {
  ...
  do.call(rbind, c(dbscan_rows, hdbscan_rows))
}
