test_that("shoal_dbscan works with matrix input", {
  x <- as.matrix(iris[, 1:4])
  res <- shoal_dbscan(x, eps = 0.5, min_samples = 5L)

  expect_s3_class(res, "shoal_dbscan")
...
test_that("shoal_dbscan works with data frame input", {
  res <- shoal_dbscan(iris, eps = 0.5, min_samples = 5L)
...
test_that("shoal_dbscan supports cosine metric", {
  x <- as.matrix(iris[, 1:4])
  res <- shoal_dbscan(x, eps = 0.01, min_samples = 5L, metric = "cosine")

  expect_s3_class(res, "shoal_dbscan")
  expect_equal(res$metric, "cosine")
})

test_that("shoal_dbscan cluster IDs are sequential from 1", {
  x <- as.matrix(iris[, 1:4])
  res <- shoal_dbscan(x, eps = 0.5, min_samples = 5L)
...
test_that("shoal_dbscan validates inputs", {
  x <- as.matrix(iris[, 1:4])

  expect_error(shoal_dbscan(1:10), "numeric matrix or data frame")
  expect_error(shoal_dbscan(x, eps = -1), "positive")
  expect_error(shoal_dbscan(x, eps = 0), "positive")
  expect_error(shoal_dbscan(x, min_samples = 0L), "positive integer")
  expect_error(shoal_dbscan(x, min_samples = 1.5), "single integer")
  expect_error(shoal_dbscan(x, metric = "manhattan"))
})

test_that("shoal_dbscan rejects data frame with < 2 numeric columns", {
  df <- data.frame(a = letters[1:10], b = 1:10)
  expect_error(shoal_dbscan(df), "at least 2 numeric columns")
})

test_that("print.shoal_dbscan returns invisibly", {
  x <- as.matrix(iris[, 1:4])
  res <- shoal_dbscan(x, eps = 0.5, min_samples = 5L)
  expect_snapshot(print(res))
})

test_that("plot.shoal_dbscan runs without error", {
...
test_that("plot.shoal_dbscan works with 2-column data", {
  skip_if_not_installed("vdiffr")
